...
function hbase_setup_namenode(){
    
    if [ -n "$OAR_FILE_NODES" ]; then
    #clean up des dossiers qui auraient été générés durant les sessions précédentes
	rm -rf ${HBASE_CONF_DIR}/* 2> /dev/null
	
	for f in ${HADOOP_USER_CONF}/hbase_conf_original/*; do
    	    cat $f | sed -e 's/\$HADOOP_NAMENODE_NAME/'$HADOOP_NAMENODE_NAME'/g' > ${HBASE_CONF_DIR}/$(basename $f);
	done;
    fi
